...
export const marketDataProvider = MARKET_DATA_PROXY
  ? 'Güvenli API proxy + Yahoo fallback'
  : TWELVE_API_KEY
    ? 'Twelve Data (geliştirme) + Yahoo fallback'
...
export async function getQuotes(symbols: string[]): Promise<Quote[]> {
  if (!hasLicensedProvider || symbols.length === 0) return getYahooQuotes(symbols);
  ...
  if (mapped.length > 0) {
    try {
      ...
      // Entitlement, symbol and quota failures fall through to the resilient provider.
    }
  }

  const missing = symbols.filter((symbol) => !collected.has(symbol));
  if (missing.length > 0) {
    const fallback = await getYahooQuotes(missing);
...
export async function getChart(symbol: string, range: ChartRange): Promise<ChartData> {
  ...
  if (!hasLicensedProvider || !mapped) return getYahooChart(symbol, range);
  try {
